(function($){
	$.fn.mdater = function(config){
		var defaults = {
			...
			marks: {
				blue_o: [],//["2016-03-03 09:00","2016-03-15 09:00"], //  正常 
				violet_o: [], // 迟到 紫色  、签卡
				yellow_o: [],  //早退  黄色
				red_o: [],    //红色，异常
				cyan_o: [],  //绿色 ，出差 、加班 、 请假、 外出
				orange_o: []   //橙色 外派
			}
		};
		...
		var mdater = {
			...
			renderHTML : function(){
				var $html = $('<div class="md_mask"></div><div class="md_panel"><div class="md_head"><div class="md_selectarea hide"><a class="md_prev change_year external" href="javascript:void(0);"></a> <a class="md_headtext yeartag external" href="javascript:void(0);"></a> <a class="md_next change_year external" href="javascript:void(0);"></a></div><div class="md_selectarea"><a class="md_prev change_month external" href="javascript:void(0);"></a> <a class="md_headtext monthtag external" href="javascript:void(0);">月</a> <a class="md_next change_month external" href="javascript:void(0);"></a></div></div><div class="md_body"><ul class="md_weekarea"><li>周日</li><li>周一</li><li>周二</li><li>周三</li><li>周四</li><li>周五</li><li>周六</li></ul><ul class="md_datearea in"></ul></div><div class="md_foot"><a href="javascript:void(0);" class="md_ok">确定</a> <a href="javascript:void(0);" class="md_cancel">取消</a></div></div>');
				if($('.md_mask').length==0){$(input).append($html);}
				return $html;
			},
			...
			getDateStr : function(y, m, d){
				...
				for(var j=startDay; j<=endDay; j++){
                    ...
                    //添加标签类名
					current = this.returnClassBymarks(this.value.year, this.value.month, j);
			...
			refreshView : function(){
				var initVal = null//this.input.val(),
					date = null;
				if(initVal){
				...
				var height = $('.md_datearea').height();
				var heightWeek = $('.md_weekarea').height();
				$(".md_body").css("height", height + heightWeek);
			},
			...
			markInit : function(){
		        ...
		        // /alert(option.marks.blue[0]);
		        for(var color in option.marks){
                     _this.marks[color] = {
                     	year: [],
                     	month: [],
                     	day: []
                     ...
                     for(var i = 0, l = option.marks[color].length; i < l; i++){
                     	var time = option.marks[color][i].split(' ')[0];
                        
                        var attr = time.split(/[-,/]/);


                        if(_this.marks[color].year.indexOf(parseInt(attr[0])) < 0){
                        	_this.marks[color].year.push(parseInt(attr[0]));
                        }
                        if(_this.marks[color].month.indexOf(parseInt(attr[1])) < 0){
                        	_this.marks[color].month.push(parseInt(attr[1]));
                        }
                        if(_this.marks[color].day.indexOf(parseInt(attr[2])) < 0){
                        	_this.marks[color].day.push(parseInt(attr[2]));
                        }
		    ...
		    //返回日期对应的样式类
		    returnClassBymarks:function(year, month, day){
		    	var _this = this;
                var className = "";
		    	for(var color in _this.marks){                     

                     if(_this.marks[color].year.indexOf(year) != -1 && _this.marks[color].month.indexOf(month+1) != -1 && _this.marks[color].day.indexOf(day) != -1){
                     	  className = color;
                     	  return className;
                     }
		    	}
